...
import { ScheduledReportFilter, ScheduledReportInfo, ScheduledReportQuery } from '@shared/models/report.models';
...
export class ScheduledReportsTableConfigResolver  {
  ...
  resolve(_route: ActivatedRouteSnapshot): EntityTableConfig<ScheduledReportInfo> {
    ...
    config.componentsData = {
      ...
      scheduledReportFilterChanged: (filter: ScheduledReportFilter) => {
        config.componentsData.scheduledReportFilter = filter;
        config.getTable().resetSortAndFilter(true);
      }
    };
  ...
  private configureEntityFunctions(config: EntityTableConfig<ScheduledReportInfo>): void {
    config.entitiesFetchFunction = pageLink => {
      const scheduledReportQuery = new ScheduledReportQuery(pageLink, {
        reportTemplateId: config.componentsData.scheduledReportFilter.reportTemplateId,
        userId: config.componentsData.scheduledReportFilter.userId,
        includeCustomers: config.componentsData.includeCustomers
      });
      return this.schedulerEventService.getScheduledReports(scheduledReportQuery);
    };
