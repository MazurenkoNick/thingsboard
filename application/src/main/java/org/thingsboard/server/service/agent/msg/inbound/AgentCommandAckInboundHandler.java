...
public class AgentCommandAckInboundHandler implements AgentInboundMessageHandler {
    ...
    public boolean canHandle(AgentInboundMsgCtx msgCtx) {
        return msgCtx != null && msgCtx.msg() != null && msgCtx.msg().hasAck();
    }

    @Override
    public void handle(AgentInboundMsgCtx msgCtx) {
        CommandAck ack = msgCtx.msg().getAck();
        log.trace("Received command ack agent inbound agent message: {}", ack);
        if (ack.getStatus() == AckStatus.REJECTED) {
            agentWorkflowProcessorService.onRejectedAck(msgCtx.session());
        }
    }
}
